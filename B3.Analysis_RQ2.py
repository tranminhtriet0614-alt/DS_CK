...
for phase_code in [1, 2, 3]:
    ...
    # Biểu đồ trực quan hóa riêng cho từng giai đoạn: tỷ lệ chuyển đổi theo kênh tiếp cận
    ...
    sns.barplot(data=contact_plot, x='Kênh tiếp cận', y='Tỷ lệ chuyển đổi', hue='Kênh tiếp cận', palette='Set2', legend=False)
    plt.title(f'Tỷ lệ chuyển đổi theo kênh tiếp cận - {phase_names[phase_code]}', fontsize=11, fontweight='bold')
    ...
    plt.close()
    print(f"   -> Đã xuất biểu đồ riêng cho giai đoạn {phase_code}: ./RQ2_contact_phase_{phase_code}.png")
